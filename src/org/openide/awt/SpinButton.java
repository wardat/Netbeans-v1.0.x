...
public class SpinButton extends Canvas {
    ...
    *
    * @param <CODE>Color color</COLOR> New foreground color.
    */
    ...
    */
    public void setValue(int aValue) {
        ...
        if ((getValue() == minimum) || (getValue() == maximum) ||
                (oldValue == minimum) || (oldValue == maximum)) {
            repaint();
        }
    }
    ...
    */
    ...
    private void paintArrow(Graphics g, int x, int y, int w, int h, boolean aDir) {
        ...
        if ((((aDir == SPIN_UP)&&(value >= maximum))||((aDir == SPIN_DOWN)&&(value <= minimum))) && !boundsIgnored) {
            ...
            g.setColor(new Color((fg.getRed() + 2*bg.getRed())/3, (fg.getGreen() + 2*bg.getGreen())/3,
    ...
    public void notifySpinButtonListenersAboutUpMove () {
        int i, k = spinButtonListeners.size ();
        for (i = 0; i < k; i ++)
