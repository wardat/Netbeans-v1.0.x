    ...
    */
    ...
    public int print(Graphics g, PageFormat pf, int pageNo) throws PrinterException {
        ...
        if (pageNo > maxPage) {
            ...
            pageno = pageNo;  //same or advance it
        } else { // skipped?
            closeDialog();
        ...
        if (cancellationPanel != null) {
            cancellationPanel.setPageno(pageNo);
            packDialog();
        }
        ...
        */  
        ...
        TextLayout headerString = new TextLayout(header.format(pageArgs),
                                  getHeaderFont(),
                                  graphics.getFontRenderContext());
        TextLayout footerString = new TextLayout(footer.format(pageArgs),
                                  getFooterFont(),
                                  graphics.getFontRenderContext());
        float pageBreakCorrection = (printHeader ? headerString.getAscent() +
                                     (headerString.getDescent() +
                                      headerString.getLeading()) * 2 : 0) +
                                    (printFooter ? footerString.getAscent() * 2 +
                                     footerString.getDescent() +
                                     footerString.getLeading() : 0);
        ...
        final float wrappingWidth = (wrap() ? (float) pf.getImageableWidth() - correction :
                                     Float.MAX_VALUE);
        final float pageBreak = ((float) pf.getImageableHeight()) - pageBreakCorrection;
    ...
    */
    private void createCancellationPanel(final PrinterJob job) {
        ...
        DialogDescriptor ddesc = new DialogDescriptor(
                                     cancellationPanel,
                                     PrintSettings.getString("CTL_Print_cancellation"),
                                     false,
                                     new Object[] {PrintSettings.getString("CTL_Cancel")},
                                     PrintSettings.getString("CTL_Cancel"),
    ...
    static final class CancellationPanel extends javax.swing.JPanel {
        ...
        */
        public CancellationPanel(PrinterJob job) {
            ...
            this.job = job;
            format = new MessageFormat(PrintSettings.getString("CTL_Print_progress"));
            msgParams = new Object[1];

            setLayout(new java.awt.GridBagLayout());
            printProgress = new javax.swing.JLabel();
            printProgress.setFont(getFontInstance());
            java.awt.GridBagConstraints gridBagConstraints = new java.awt.GridBagConstraints();
            add(printProgress, gridBagConstraints);
        }
        ...
        */
        public void setPageno(int pageno) {
            msgParams[0] = new Integer(pageno);
            printProgress.setText(format.format(msgParams));
        }
        /** @return preferred size which is greater than computed by awt */
 ...
 *  13   src-jtulach1.12        10/26/99 Ales Novak      #3732
 ...
 *       UID
 *  10   src-jtulach1.9         7/7/99   Jesse Glick     Context help.
 *  9    src-jtulach1.8         6/8/99   Ian Formanek    ---- Package Change To 
 *       org.openide ----
 *  8    src-jtulach1.7         6/8/99   Ales Novak      IllegalArgumentException
 *       from Bidi
 *  7    src-jtulach1.6         6/4/99   Ales Novak      # 1970
 *  6    src-jtulach1.5         5/12/99  Ales Novak      cancellation featurefix
 *  5    src-jtulach1.4         5/11/99  Ales Novak      filenames used + footer 
 *       bug + line ascent correction + cancel dialog
 *  4    src-jtulach1.3         4/30/99  Ales Novak      options take part in 
 *       printing
 *  3    src-jtulach1.2         4/23/99  Ales Novak      color added
 *  2    src-jtulach1.1         4/21/99  Ales Novak      first version of 
 *       printing
