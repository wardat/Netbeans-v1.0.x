...
public class SystemActionTesterBeanInfo extends SimpleBeanInfo {

    public BeanDescriptor getBeanDescriptor () {
        BeanDescriptor bd = new BeanDescriptor (SystemActionTester.class);
        bd.setDisplayName ("Test SystemAction Objects");
        return bd;
    }
    ...
    public Image getIcon (int type) {
        if (type == BeanInfo.ICON_COLOR_16x16 || type == BeanInfo.ICON_MONO_16x16) {
            if (icon == null)
                icon = loadImage ("resources/SystemActionTesterIcon.gif");
            return icon;
        } else {
            return null;
        }
