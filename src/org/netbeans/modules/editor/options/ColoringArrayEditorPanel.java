...
public class ColoringArrayEditorPanel extends javax.swing.JPanel {
    ...
    public void setValue( HashMap map ) {
        ...
        for( i = 0; i < names.length; i++ ) {
            ...
            if( visualNames[i] == null ) visualNames[i] = LocaleSupport.getString("NAME_coloring_" + BaseOptions.BASE + "_" + names[i], names[i] ); // NOI18N
        }
    ...
    void setEditorValue( int index ) {
        ...
        if( example == null )  example = LocaleSupport.getString("EXAMPLE_coloring_" + BaseOptions.BASE + "_" + names[index], names[index] ); // NOI18N
...
/*
 * Log
 *  6    Gandalf-post-FCS1.3.1.1     4/5/00   Petr Nejedly    Proper handling of 
 *       localized options
