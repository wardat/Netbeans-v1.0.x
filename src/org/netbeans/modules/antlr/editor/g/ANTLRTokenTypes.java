/*
 *                 Sun Public License Notice
 * 
 * The contents of this file are subject to the Sun Public License
 * Version 1.0 (the "License"). You may not use this file except in
 * compliance with the License. A copy of the License is available at
 * http://www.sun.com/
 * 
 * The Original Code is Forte for Java, Community Edition. The Initial
 * Developer of the Original Code is Sun Microsystems, Inc. Portions
 * Copyright 1997-2000 Sun Microsystems, Inc. All Rights Reserved.
 */

package org.netbeans.modules.antlr.editor.g;

public interface ANTLRTokenTypes {
    public static final int EOF = 1;
    public static final int NULL_TREE_LOOKAHEAD = 3;
    public static final int C_TEXT = 4;
    public static final int LITERAL_tokens = 5;
    public static final int LITERAL_options = 6;
    public static final int DOC_COMMENT = 7;
    public static final int OPTIONS = 8;
    public static final int TOKENS = 9;
    public static final int SEMPRED = 10;
    public static final int WS = 11;
    public static final int COMMENT = 12;
    public static final int SL_COMMENT = 13;
    public static final int ML_COMMENT = 14;
    public static final int COMMA = 15;
    public static final int QUESTION = 16;
    public static final int TREE_BEGIN = 17;
    public static final int LPAREN = 18;
    public static final int RPAREN = 19;
    public static final int COLON = 20;
    public static final int STAR = 21;
    public static final int PLUS = 22;
    public static final int ASSIGN = 23;
    public static final int IMPLIES = 24;
    public static final int SEMI = 25;
    public static final int CARET = 26;
    public static final int BANG = 27;
    public static final int OR = 28;
    public static final int WILDCARD = 29;
    public static final int RANGE = 30;
    public static final int NOT_OP = 31;
    public static final int RCURLY = 32;
    public static final int CHAR_LITERAL = 33;
    public static final int STRING_LITERAL = 34;
    public static final int ESC = 35;
    public static final int DIGIT = 36;
    public static final int XDIGIT = 37;
    public static final int VOCAB = 38;
    public static final int INT = 39;
    public static final int ARG_ACTION = 40;
    public static final int NESTED_ARG_ACTION = 41;
    public static final int ACTION = 42;
    public static final int NESTED_ACTION = 43;
    public static final int TOKEN_REF = 44;
    public static final int RULE_REF = 45;
    public static final int WS_LOOP = 46;
    public static final int INTERNAL_RULE_REF = 47;
    public static final int WS_OPT = 48;
    public static final int NOT_USEFUL = 49;
}
