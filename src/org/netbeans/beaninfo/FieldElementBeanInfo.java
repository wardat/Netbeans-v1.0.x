...
public class FieldElementBeanInfo extends MemberElementBeanInfo {
    ...
    /** no-arg */
    public FieldElementBeanInfo() {
        if (icon == null) {
            icon = loadImage("/org/openide/resources/src/variablePublic"); // NOI18N
            icon32 = icon;
        }
    }
        ...
        if (desc == null) {
            synchronized (FieldElementBeanInfo.class) {
                if (desc == null) {
                    ...
                    try {
                        desc[inh.length] = new PropertyDescriptor(PROP_TYPE, FieldElement.class, "getType", "setType"); // NOI18N
                        desc[inh.length].setPropertyEditorClass(TypeEditor.class);
                        desc[inh.length + 1] = new PropertyDescriptor(PROP_INIT_VALUE, FieldElement.class, "getInitValue", "setInitValue"); // NOI18N
