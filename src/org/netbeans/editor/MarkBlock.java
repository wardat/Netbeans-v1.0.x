...
public class MarkBlock {
    ...
    */
    ...
    throws BadLocationException {
        ...
        try {
            doc.op.insertMark(startMark, startPos);
            try {
                doc.op.insertMark(endMark, endPos);
    ...
    */
    public int extendStart(int startPos) throws BadLocationException {
        try {
            ...
            if (startPos != markPos) {
                doc.op.moveMark(startMark, startPos);
            }
            return startPos;
        } catch (InvalidMarkException e) {
            if (Boolean.getBoolean("netbeans.debug.exceptions")) { // NOI18N
 ...
 *  7    Gandalf   1.6         10/4/99  Miloslav Metelka 
 ...
 *       improvements
 *  2    Gandalf   1.1         4/8/99   Ian Formanek    Removed debug prints
